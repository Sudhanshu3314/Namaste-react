...
const EachTypeFoodList = (props) => {
	const { eachItemData, openDropDown, setOpenDropDown } = props;
	console.log("EachTypeFoodList : ", eachItemData?.card?.card);
	...
	const { title, itemCards } = eachItemData?.card?.card;
	return (
		<div>
			<div className={eachTypeFoodListStyle}>
				<div
					className="font-bold cursor-pointer flex justify-between transition-all duration-500 text-red-800 hover:text-gray-600"
					onClick={scrollDownUp}
				>
					<h1>
